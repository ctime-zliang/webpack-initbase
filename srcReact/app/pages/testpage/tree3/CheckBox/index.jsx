				...
				<div className={'wasabi-form-group-body' + (props.readOnly || props.disabled ? ' readOnly' : '')}>
					<ul className="wasabi-checkul">
						{data.map((child, index) => {
							...
							return (
								<li key={index} onClick={onSelect.bind(this, child.value ?? '', child.text ?? '', child, value, text)}>
									<label
										className={'checkbox-label ' + (checked ? ' checked ' : ' ') + (half ? ' halfcheck ' : '')}
										readOnly={readOnly}
									></label>
									<div className={'checktext ' + (checked ? ' checked' : '')} readOnly={readOnly}>
										{child.text}
									</div>
								</li>
							)
