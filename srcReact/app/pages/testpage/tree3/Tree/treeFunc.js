	...
	try {
		if (Array.isArray(data)) {
			...
			if (node) {
				...
				//设置节点及子节点的勾选
				if (isChecked && checkType && checkType.y && checkType.y.indexOf('s') > -1) {
					node = setNodeChildrenChecked(node, node.isChecked)
				} else if (!isChecked && checkType && checkType.n && checkType.n.indexOf('s') > -1) {
					node = setNodeChildrenChecked(node, node.isChecked)
				}
				//设置祖先节点
				if (node.isChecked && checkType && checkType.y && checkType.y.indexOf('p') > -1) {
					setNodeParentsChecked(findLinkNodesByPath(data, node._path))
				} else if (!node.isChecked && checkType && checkType.n && checkType.n.indexOf('p') > -1) {
					setNodeParentsChecked(findLinkNodesByPath(data, node._path))
				}
			}
		}
		...
		if (Array.isArray(data)) {
			...
			if (node) {
				...
				if (nodes.length === 1) {
					...
					parentRemoveNode.children = setChildrenPath(hashData, parentRemoveNode.id, parentRemoveNode._path, parentRemoveNode.children)
				}
				return data
			}
		}
	...
	if (Array.isArray(data)) {
		...
		if (node) {
			...
			if (Array.isArray(newNode[options?.childrenField] ?? newNode.children)) {
				//如果有子节点，则更新子节点,设置路径
				node.children = setChildrenPath(hashData, node.id, node._path, newNode.children, options)
			}
...
export function moveInNode(hashData, data, dragNode, dropNode, options) {
	...
	try {
		...
		if (dragNode && dropNode && dropNodes && dragNode.pId !== dropNode.id && dropNodes.filter(node => node.id === dragNode.id).length === 0) {
			/**
			...
			dropNode.children = setChildrenPath(hashData, dropNode.id, dropNode._path, dropNode.children, options)
		} else {
			console.log('停靠的节点不能是移动节点的父节点或者停靠的节点不能是移动节点的子节点')
		}
	} catch (e) {
 ...
 * @returns
 */
function moveBeforeOrAfterNode(hashData, data, dragNode, dropNode, options, step = 0) {
	try {
		//在数据中找到节点，重新查找节点，这样才能保证节点在数据上
		dragNode = findNodeById(hashData, data, dragNode?.id)
		dropNode = findNodeById(hashData, data, dropNode?.id)
		let dropNodes = findLinkNodesByPath(data, dropNode._path) //停靠节点链表
		if (dragNode && dropNode && dropNodes && dropNodes.filter(node => node.id === dragNode.id).length === 0) {
			/**
			...
			if (dropNodes.length === 1) {
				...
				parentDropNode.children = setChildrenPath(hashData, parentDropNode.id, parentDropNode._path, parentDropNode.children, options)
			}
		} else {
 ...
 * @param {*} options 属性选项
 */
export function setChildrenPath(hashData = new Map(), pId, path, children, options) {
		...
		if (Array.isArray(children)) {
			for (let i = 0; i < children.length; i++) {
				...
				if (Array.isArray(children[i][childrenField])) {
					children[i].children = setChildrenPath(hashData, children[i].id, newPath, children[i][childrenField], options)
				}
			}
			return children
		}
...
export function appendChildren(hashData, data = [], pId, children, options) {
	try {
		if (Array.isArray(children)) {
			...
			//去掉id已经存在的节点
			let newChildren = children.filter(item => !hashData.has(item[options?.idField] ?? item?.id))
			...
			if (parentNode) {
				...
				// 设置节点路径
				parentNode.children = setChildrenPath(hashData, parentNode.id, parentNode._path, parentNode.children, options)
