...
import { findNodeById, findLinkNodesByPath, getChecked, setChildrenPath } from './treeFunc'
...
const TreeContainer = React.forwardRef(function (
	...
	const onTreeExpand = useCallback(
		(isOpened, id, text, node) => {
			// 先设置折叠或者展开
			handlerData(gobalData, { type: 'setOpen', payload: { id, isOpened, foldBroAble } }, dispatch)
			...
			if (startIndex >= 1) {
				...
				const size =
					...
					(startIndex - config.bufferScale * visibleDataCount >= 0
						? (startIndex - config.bufferScale * visibleDataCount) * config.rowDefaultHeight
						: 0)
...
// */
// 		findNode(id) {
// 			return findNodeById(gobalData.current.hashData, gobalData.current.data, id)
// 		},
// 		/** 
// 获取某个节点整个链路树
// * @param {*} id
// */
// 		findParents(id) {
// 			const node = findNodeById(gobalData.current.hashData, gobalData.current.data, id)
// 			return node && node._path ? findLinkNodesByPath(gobalData.current.data, node._path) : []
// 		},
// 		/*获取所有节点
// @returns
// */
...
// */
// 		remove(ids) {
// 			handlerData(gobalData, { type: 'remove', payload: { ids, options } }, dispatch)
// 		},
// 		/**
// 移除所有
...
// 			if (nodes) {
// 				//格式化节点防止有些属性没传而影响后
// 				handlerData(gobalData, { type: 'update', payload: { nodes: nodes, options } }, dispatch)
// 			}
// 		},
// 		/**
// 更新所有
...
// 	}))
	...
	useEffect(() => {
		if (url) {
			...
			getData(url, httpType, contentType, headers, params, res => {
				gobalData.current.data = setChildrenPath(gobalData.current.hashData, '', [], handerLoadData(res, dataSource, loadSuccess), options)
				gobalData.current.flatData = treeDataToFlatData(gobalData.current.data)
				onScroll()
			})
		} else {
			//注意了，空数据也可以
			gobalData.current.data = setChildrenPath(gobalData.current.hashData, '', [], data, options)
			gobalData.current.flatData = treeDataToFlatData(gobalData.current.data)
			onScroll()
		}
	}, [data, url])
	...
	useEffect(() => {
		if (state.scrollIndex && state.scrollIndex.index >= 0) {
			const vis = getVisibleCount(treecontainerid)
			if (state.scrollIndex.index > vis.endIndex || state.scrollIndex.index < vis.startIndex) {
				//不在可见范围内
				document.getElementById(treecontainerid).scrollTop = (state.scrollIndex.index - 1) * config.rowDefaultHeight
			}
		}
	}, [state.scrollIndex])
	/**
	...
	if (!componentType || componentType === 'tree') {
		control = <TreeView {...treeProps} {...state} {...treeEvents} treeid={treeid}></TreeView>
	...
	return (
		<div id={treecontainerid} onScroll={onScroll} className={'wasabi-tree-parent ' + (className ?? '')} style={style}>
			{control}
			<div
				style={{
					left: 0,
					top: 0,
					height: gobalData.current.flatData && gobalData.current.flatData.length * config.rowDefaultHeight,
					position: 'absolute',
					width: 1,
				}}
			></div>
