...
function TreeView(props) {
	let nodeControl = []
	//得到传下去的属性
	const { componentType, selectAble, checkStyle, renameAble, removeAble, draggAble, dropAble, dropType, clickId, loadingId } = props
	...
	if (Array.isArray(data)) {
		nodeControl = data.map((item, index) => {
			...
			if (item.isParent === true || (Array.isArray(item.children) && item.children.length > 0)) {
				//如果明确规定了，或者子节点不为空，则设置为父节点
				isParent = true
			}
			return <TreeNode key={'treenode-' + item.pId + '-' + item.id} {...treeProps} {...item} isParent={isParent} {...treeEvents} />
		})
	}
	return (
		<ul id={props.treeid} className={'wasabi-tree clearfix ' + (props.dottedAble === false ? ' nodotted ' : '')}>
			{nodeControl}
		</ul>
	)
}
