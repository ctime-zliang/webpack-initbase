...
function NodeView(props) {
	...
	if (row.isParent) {
		...
		if (row.isOpened) {
			if (props.arrowUnFoldIcon) {
				arrowIcon = (
					<div className={'wasabi-tree-li-icon'} style={{ display: 'inline-block' }} onClick={row.isParent ? onExpand : null}>
			...
			arrowIcon = (
				<i
					className={
						...
						(row.open ? ` wasabi-tree-li-icon  ${icon}-down ` : ` wasabi-tree-li-icon  ${icon}-right`)
					}
				...
				>
					...
					<span
						...
						style={{
							height: (childrenLength + 1) * config.rowDefaultHeight + (row._isLast ? config.rowDefaultHeight * -1 : 15),
						}}
					></span>
				</i>
			)
	...
	//节点元素
	return (
		<li className="wasabi-tree-li" key={row.pId + '-' + row.id} style={{ display: row.hide ? 'none' : 'flex' }}>
			{blankControl}
			{/* 折叠节点 */}
			{/* //  不是父节点也要一个占位符 */}
			{arrowIcon}
			...
			<div
				...
				className={clickId === row.id ? 'wasabi-tree-li-node selected' : 'wasabi-tree-li-node'}
				title={title}
				onDrop={onNodeDrop}
			...
			>
				{rename ? (
					<Text id={row.textid} required={true} onKeyUp={onKeyUp} onBlur={onBlur} name={'key' + row.id} value={row.text}></Text>
				) : (
					<div
						key="2"
					...
					>
						...
						<i
							key="3"
							className={(!row.selectAble && childrenLength ? ' noCheckhasChildren ' : '  ') + iconCls + ' wasabi-tree-text-icon'}
						></i>
