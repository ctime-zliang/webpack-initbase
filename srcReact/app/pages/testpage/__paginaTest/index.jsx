	...
	render() {
		...
		if (!props.diableInputPaginate) {
			inputDom = (
				<li className="page-input-wrap">
					...
					<button className="page-input-btn" onClick={this._onInputOk.bind(this)}>
		...
		return (
			<ul className={wrapCls}>
				<li onClick={this._handlePrev.bind(this)} className={prevCls}>
				...
				<li onClick={this._handleNext.bind(this)} className={nextCls}>
					<span className="page-link-btn" href="">
		...
		if (props.pageNum <= props.middleDisplayRange) {
			...
			for (var i = 0; i < props.pageNum; i++) {
				...
				if (lastBreakItem !== breakItem) {
					breakItem = <li className="break">{props.breakLabel}</li>
